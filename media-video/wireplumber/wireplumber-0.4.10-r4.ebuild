...
PATCHES=(
	"${FILESDIR}"/${PN}-0.4.10-config-fix-enabled-property-to-default-to-true-when.patch
	"${FILESDIR}"/${PN}-0.4.10-m-lua-scripting-allow-converting-GValue-holding-NUL.patch
	"${FILESDIR}"/${PN}-0.4.10-alsa.lua-fix-device-name-deduplication-when-reserva.patch
	"${FILESDIR}"/${PN}-0.4.10-m-default-nodes-don-t-check-if-all-device-nodes-are.patch
	"${FILESDIR}"/${PN}-0.4.10-m-lua-scripting-fix-object-refcounting.patch
)
